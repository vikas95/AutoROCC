...
def main():
    myPlot("Human", '-r', 1.5, 'baseline-scores/human-01.json')
    myPlot("IR(Paragraphs)", '-.p', 1.5, 'baseline-scores/lucene_paragraphs.json')
    myPlot("IR(World)", '*-b', 1.5, 'baseline-scores/lucene_world.json')
    myPlot("SimpleLR", '^--c', 1.5, 'baseline-scores/simpleLR.json')
    myPlot("Gold_sentences", '^--b', 1.5, 'baseline-scores/gold_results.json')
    myPlot("full_passage_sentences", '^--m', 1.5, 'baseline-scores/full_passage_results.json')
    myPlot("POCC_4_SENT", '^--g', 1.5, 'baseline-scores/POCC_4SENT_results.json')
